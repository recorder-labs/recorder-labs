...
function _savePracticeStars() {
  const today = new Date().toISOString().slice(0, 10);
  const data = { date: today, stars: JSON.parse(JSON.stringify(PRACTICE_STARS)) };
  try { localStorage.setItem('recorder_practice_stars', JSON.stringify(data)); } catch(e) {}
}

function _loadPracticeStars() {
  try {
    const raw = localStorage.getItem('recorder_practice_stars');
    if (!raw) return;
    const data = JSON.parse(raw);
    const today = new Date().toISOString().slice(0, 10);
    if (data.date === today && data.stars) {
      Object.keys(PRACTICE_STARS).forEach(key => {
        if (typeof data.stars[key] === 'number') {
          PRACTICE_STARS[key] = data.stars[key];
        }
      });
    } else {
      localStorage.removeItem('recorder_practice_stars');
    }
  } catch(e) {}
}
